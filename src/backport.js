...
const getBaseBranchFromLabel = (label, labelRegExp) => {
    ...
    if (!base) {
        throw new Error(`RegExp "${String(labelRegExp)}" matched "${label}" but missed a "base" named capturing group.`);
    }
    return base;
};
const getBaseBranches = ({ labelRegExp, payload, }) => {
    if ("label" in payload) {
        const base = getBaseBranchFromLabel(payload.label.name, labelRegExp);
        return base ? [base] : [];
    }
    return compact(payload.pull_request.labels.map((label) => getBaseBranchFromLabel(label.name, labelRegExp)));
};
const warnIfSquashIsNotTheOnlyAllowedMergeMethod = async ({ github, owner, repo, }) => {
    const { data: { allow_merge_commit, allow_rebase_merge }, } = await github.request("GET /repos/{owner}/{repo}", { owner, repo });
...
const backportOnce = async ({ base, body, commitSha, github, head, labels, owner, repo, title, }) => {
...
const getFailedBackportCommentBody = ({ base, commitSha, errorMessage, head, }) => {
...
const backport = async ({ getBody, getHead, getLabels, getTitle, labelRegExp, payload, token, }) => {
    const { pull_request: { body: originalBody, labels: originalLabels, merge_commit_sha: mergeCommitSha, merged, number, title: originalTitle, }, repository: { name: repo, owner: { login: owner }, }, } = payload;
    if (merged !== true || !mergeCommitSha) {
        // See https://docs.github.com/en/actions/using-workflows/events-that-trigger-workflows#pull_request_target.
        throw new Error("For security reasons, this action should only run on merged PRs.");
    }
